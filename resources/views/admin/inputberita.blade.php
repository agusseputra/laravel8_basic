...
<!-- action menuju fungsi store pada controller berita
data dikirim menggunakan methode post dan
enctype multipart/form-data untuk upload file
nama route disesuaikan dengan yang ada di route lits-->
<!-- untuk mengenali update atau create gunakan sebuah variabel $berita,
$berita ini akan digunakan selalau pada halaman ini-->
<!-- pada action perlu diberikan kondisi yang tepat
agar action tepat sasaran create atau update-->
<!-- jika ditemukan variabel $berita maka route yang digunakan adalah berita.update-->
<form action="{{(isset($berita))?route('berita.update', $berita->id):route('berita.store')}}" method="POST" enctype="multipart/form-data">
@csrf
<!--standar laravel utk update menggunakan method PUT,
jd jika ditemukan variabel berita maka method PUT dipanggil-->
@if (isset($berita))
@method('PUT')
@endif
...
<div class="grid grid-cols-3 gap-6">
    ...
    <div class="mt-1 flex rounded-md shadow-sm">
        <!-- fungsi old('title') untuk memberikan value default untuk text jika terjadi validasi error,
            ...
            <!-- jika ditemukan variabel berita, maka tampilkan title yang dikirim dari controller,
            jika tidak maka tampilkan default old(title)-->
...
<div>
    ...
    <div class="space-y-1 text-center">
        <!-- jika ditemukan variabel berita maka tampilkan cover-->
